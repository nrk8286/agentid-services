...
const CUEKEEPER_X402_PRODUCT = {
  id: "cuekeeper_agent_foundry_release",
  name: "CueKeeper",
  priceLabel: "$1 USDC",
  checkoutUrl: "https://score.agentid.life/cuekeeper?utm_source=gptmarketplus.com&utm_medium=owned&utm_campaign=agent_foundry_release&utm_content=product_catalog",
  summary: "A local Python toolkit that converts SRT and WebVTT subtitles into timestamp-preserving transcripts with deterministic JSON quality reports.",
  includes: [
...
const RESOURCE_PAGES = [
  ...
  },
  {
    ...
    category: "Guide",
    title: "AI Agents for Small Business: Costs, Uses & $29 Kit",
    description: "Compare small-business AI-agent costs and use cases, then map one bounded workflow with the $29 Launch Kit, starter prompt, intake fields, QA, and a 30-day scorecard.",
  ...
  },
  {
    ...
    category: "Cost guide",
    title: "AI Receptionist Pricing (2026): Costs, Plans & First Workflow",
    description: "Compare AI receptionist prices, usage, setup, integrations, and support, then map a bounded first workflow with the $29 Launch Kit before custom implementation.",
...
function pageEntriesForSitemap(env) {
  const pages = [
    ...
    { path: "/ai-marketing-automation", title: "AI Marketing Automation for Small Business", description: "AI marketing automation for lead capture, qualification, follow-up, sales handoff, and measurable results, with a $29 Launch Kit path for one bounded workflow." },
    ...
    { path: "/ai-receptionist-software", title: "AI Receptionist Software Comparison", description: "Compare AI receptionist software by channel, booking, handoff, compliance, resale, and price, then map a bounded first workflow with the $29 Launch Kit." },
...
function renderLlmsTxt(env) {
  ...
  return [
    ...
    "",
    "## Agent Foundry releases",
    `- [Supabase Recovery Evidence Agent guide](${siteUrl(env)}/guides/supabase-project-recovery-evidence): Build a redacted incident evidence packet and review the separately hosted $1 USDC local utility. The utility does not access Supabase, repair projects, determine root cause, or contact support.`,
    `- [Agent Foundry products](${siteUrl(env)}/products): Compare the separately hosted $1 USDC recovery agent and CueKeeper releases with the PayPal-gated GPTMarketPlus products.`,
    "",
    "## Conversion path",
...
function renderLlmsFullTxt(env) {
  ...
  return [
    ...
    "",
    `${brandName(env)} leads with a $29 AI Agent Launch Kit for a usable first workflow, then offers separately scoped implementation for businesses that need installation or integrations. Its products marketplace also links to separately hosted $1 USDC Agent Foundry releases, including the local, credential-free Supabase Recovery Evidence Agent.`,
    "",
    pages,
    "",
    "## Private pages",
