...
PayPal is the sole payment provider processed by this Worker. Eligible self-service products use PayPal Orders, while approved services and sponsor placements use PayPal checkout, subscriptions, or invoices only after scope and fulfillment terms are accepted. The marketplace also links to CueKeeper, a separately hosted Agent Foundry release whose own service clearly labels and verifies its $1 Base USDC x402 checkout.

The `/products` marketplace lists seven one-time products: the $49 Auto Dropshipping Agent Team, $49 Local Lead Follow-Up Agent Team, $39 Content & SEO Agent Team, $39 Ecommerce Support Agent Team, $29 AI Agent Launch Kit, $24 AI Software Opportunity Report, and the separately hosted $1 USDC CueKeeper utility. The four agent teams and Launch Kit open private, order-bound workspaces only after the Worker verifies a completed PayPal capture and random access token. The three newer teams share the hardened workspace API at `/api/paypal/agent-team/workspace`; each produces a product-specific, human-reviewed operating pack that can be saved, copied, or downloaded. Private workspace pages are `noindex` and `no-store`, and provider refunds or reversals can revoke the underlying entitlement. CueKeeper is not submitted to this Worker's PayPal order endpoint; its Agent Foundry storefront owns settlement and ZIP delivery.
