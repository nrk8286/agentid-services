...
PayPal is the sole payment provider processed by this Worker. Eligible self-service products use PayPal Orders, while approved services and sponsor placements use PayPal checkout, subscriptions, or invoices only after scope and fulfillment terms are accepted. The marketplace also links to separately hosted Agent Foundry releases whose own service clearly labels and verifies each $1 Base USDC x402 checkout.

The `/products` marketplace lists eight one-time products: the $49 Auto Dropshipping Agent Team, $49 Local Lead Follow-Up Agent Team, $39 Content & SEO Agent Team, $39 Ecommerce Support Agent Team, $29 AI Agent Launch Kit, $24 AI Software Opportunity Report, the separately hosted $1 USDC CueKeeper utility, and the separately hosted $1 USDC Supabase Recovery Evidence Agent. The four agent teams and Launch Kit open private, order-bound workspaces only after the Worker verifies a completed PayPal capture and random access token. The three newer teams share the hardened workspace API at `/api/paypal/agent-team/workspace`; each produces a product-specific, human-reviewed operating pack that can be saved, copied, or downloaded. Private workspace pages are `noindex` and `no-store`, and provider refunds or reversals can revoke the underlying entitlement. Agent Foundry releases are not submitted to this Worker's PayPal order endpoint; their own storefront verifies x402 settlement and delivers the ZIP.

[CueKeeper](https://score.agentid.life/cuekeeper?ref=github-readme) converts existing SRT or WebVTT files into local, analysis-ready Markdown or plain text while retaining time references. The $1 USDC ZIP includes an optional dialogue-only output and a JSON quality report; it does not transcribe or translate media.
...
The $29 AI Agent Launch Kit uses a verified live PayPal order/capture flow. Completed captures return a private Launch Kit Workspace immediately, send that workspace URL through the customer transactional-email adapter, and queue failed email delivery for retry without duplicating successful delivery. The workspace turns buyer inputs into a launch setup brief, tailored starter prompt, workflow map, lead-intake plan, follow-up sequence, scenario-based launch QA checklist, and 30-day scorecard; the buyer can copy or download the generated starter pack. Four core fields are required to start, while launch channel, success metric, current baseline, and 30-day target make the resulting plan more executable when the buyer has those details. The original Markdown workbook remains available as a legacy download. For Google measurement, the site uses `GOOGLE_TAG_GATEWAY_PATH=/gtag` and serves that path from the Worker itself.
