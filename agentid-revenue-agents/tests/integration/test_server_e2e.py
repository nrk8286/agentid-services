    ...
    data = {
        ...
        "new_message": {
            ...
            "parts": [
                {
                    "text": (
                        "Give me a zero-cost AgentID growth readiness check. "
                        "Do not claim a dry-run was executed."
                    )
                }
            ],
        },
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    message = Message(
        ...
        parts=[
            Part(
                root=TextPart(
                    text=("Summarize AgentID revenue readiness without spending money.")
                )
            )
        ],
    )
...
@pytest.mark.live_model
def test_scheduled_revenue_pubsub_trigger(
    server_fixture: subprocess.Popen[str],
) -> None:
    """Test the six-hour revenue-cycle event through ADK's Pub/Sub endpoint."""
    event = {
        ...
        "constraints": {
            "dry_run": True,
            "daily_spend_limit_cents": 1_000,
            "monthly_spend_limit_cents": 10_000,
        },
    }
    payload = {
        "message": {
        ...
        "subscription": "agentid-revenue-cycle-test",
    }
    response = requests.post(
        f"{BASE_URL}/apps/app/trigger/pubsub",
        json=payload,
        headers=HEADERS,
        timeout=90,
    )
